...
bool AprsPlugin::render( GeoPainter *painter, ViewportParams *viewport, const QString& renderPos, GeoSceneLayer * layer )
{
    ...
    if ( !( viewport->viewLatLonAltBox() == m_lastBox ) ) {
        ...
        QString towrite = QLatin1String("#filter a/") +
            QString::number(m_lastBox.north().toDegree()) + QLatin1Char('/') +
            QString::number(m_lastBox.west().toDegree())  + QLatin1Char('/') +
            QString::number(m_lastBox.south().toDegree()) + QLatin1Char('/') +
            QString::number(m_lastBox.east().toDegree())  + QLatin1Char('\n');
