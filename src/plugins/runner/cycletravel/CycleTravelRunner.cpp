...
QVector<QPair<GeoDataCoordinates,QString> > CycleTravelRunner:: m_cachedHints;

QString CycleTravelRunner:: m_hintChecksum;

CycleTravelRunner::CycleTravelRunner( QObject *parent ) :
...
CycleTravelRunner::~CycleTravelRunner()
{
    // nothing to do
}

void CycleTravelRunner::retrieveRoute( const RouteRequest *route )
{
    if ( route->size() < 2 ) {
        return;
    }

    QString url = "http://route-eu.cycle.travel/viaroute?output=json&instructions=true";
    ...
    m_request.setRawHeader( "User-Agent", TinyWebBrowser::userAgent( "Browser", "CycleTravelRunner" ) );
...
void CycleTravelRunner::retrieveData( QNetworkReply *reply )
{
    if ( reply->isFinished() ) {
...
void CycleTravelRunner::handleError( QNetworkReply::NetworkError error )
{
    mDebug() << " Error when retrieving CycleTravel route: " << error;
}

void CycleTravelRunner::get()
{
...
void CycleTravelRunner::append(QString *input, const QString &key, const QString &value)
{
    *input += '&' + key + '=' + value;
}

GeoDataLineString *CycleTravelRunner::decodePolyline( const QString &geometry )
{
...
RoutingInstruction::TurnType CycleTravelRunner::parseTurnType( const QString &instruction )
{   
    if ( instruction == "1" ) {
...
GeoDataDocument *CycleTravelRunner::parse( const QByteArray &input ) const
{
    ...
    if ( data.property( "route_geometry" ).isString() ) {
        ...
        const QString name = nameString( "CycleTravel", length, time );
